...
class Album_ind(models.Model):
    ...
    album_poster = models.ImageField('Обложка альбома', upload_to='individualnaya_fotosessiya/')
...
class Foto_ind(models.Model):
    image = models.ImageField('Сжатая фотография', db_index=False,
                              upload_to='individualnaya_fotosessiya/albums')
    image_full = models.ImageField('Фото с хорошим качеством', blank=True, db_index=False,
                                   upload_to='individualnaya_fotosessiya/albums/low')
